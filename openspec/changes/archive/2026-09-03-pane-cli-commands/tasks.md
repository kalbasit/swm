...
- [x] 1.1 (`cmd/swm`) Write tests for a new `internal/exitcode` package: a nil
...
- [x] 1.2 (`cmd/swm`) Add `internal/exitcode` with `Coder`, `Error`, and
      `From`. No `os.Exit` in the package — it only computes the status.
- [x] 1.3 (`cmd/swm`) Teach `main` to exit with `exitcode.From(err)` instead of
      a hard-coded 1.

## 2. Pane JSON encoding

- [x] 2.1 (`cmd/swm`) Write tests for the pane JSON view: snake_case keys, all
      fields present when the descriptive ones are empty, and an empty list
      encoding as an empty array rather than null.
- [x] 2.2 (`cmd/swm`) Add `internal/cli/pane` with the JSON view struct and its
      encoding helpers.

## 3. swm pane open

- [x] 3.1 (`cmd/swm`) Write tests: argv passed through verbatim including an
...
- [x] 3.2 (`cmd/swm`) Implement `pane open`.

## 4. swm pane list

- [x] 4.1 (`cmd/swm`) Write tests: no filters, both filters forwarded, JSON
      array output, empty stream yields `[]` in JSON mode and no output in
      table mode, focused flag preserved, stream error surfaced.
- [x] 4.2 (`cmd/swm`) Implement `pane list`, including the table renderer.

## 5. swm pane send

- [x] 5.1 (`cmd/swm`) Write tests: text delivered, submit-only, nothing-to-send
...
- [x] 5.2 (`cmd/swm`) Implement `pane send`.

## 6. swm pane close

- [x] 6.1 (`cmd/swm`) Write tests: close forwards workspace and pane, an
      already-closed pane succeeds, an error is surfaced.
- [x] 6.2 (`cmd/swm`) Implement `pane close`.

## 7. Wiring and end-to-end coverage

- [x] 7.1 (`cmd/swm`) Register the `pane` group in `internal/cli/root.go`.
- [x] 7.2 (`cmd/swm`) Add integration tests in `cmd/swm/tests/integration`
      ...
      status 3.
- [x] 7.3 (root) Run `task fmt`, `task lint`, and `task test`; all must exit 0.
