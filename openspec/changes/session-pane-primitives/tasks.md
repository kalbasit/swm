...
- [x] 1.1 (`proto`) Add `Pane`, `OpenPaneRequest`, `ListPanesRequest`,
...
- [x] 1.2 (`proto`) Run `task proto:gen` and commit the regenerated
      `session.pb.go` / `session_grpc.pb.go`. Verify `task proto:lint` and
      `task proto:build` pass.
- [x] 1.3 (root) Run `task update-nix-vendor-hashes` — required after any
...
- [x] 2.1 (`plugins/session-tmux`) Write tests for a new `internal/shellquote`
      package covering: bare word unchanged, empty string, spaces, embedded
      single quotes, shell metacharacters, and argv joining.
- [x] 2.2 (`plugins/session-tmux`) Move `shellQuote` out of `internal/layout`
...
- [x] 3.1 (`plugins/session-tmux`) Extend `internal/session/testdata/faketmux`
...
- [x] 4.1 (`plugins/session-tmux`) Write tests: all panes across two live
...
- [x] 4.2 (`plugins/session-tmux`) Extract the live-socket scan out of
      `ListWorkspaces` into a shared helper and implement `panes(ctx, sock)`
      over `list-panes -a -F`, plus `Tmux.ListPanes` on top of both.

## 5. OpenPane

- [x] 5.1 (`plugins/session-tmux`) Write tests: argv with a spaced element is
...
- [x] 5.2 (`plugins/session-tmux`) Implement `Tmux.OpenPane`.

## 6. SendText

- [x] 6.1 (`plugins/session-tmux`) Write tests: literal text plus separate
...
- [x] 6.2 (`plugins/session-tmux`) Implement `Tmux.SendText` using
      `send-keys -l --` for the text and a separate `send-keys Enter` for
      submit, gated on the shared pane query.

## 7. ClosePane

- [x] 7.1 (`plugins/session-tmux`) Write tests: live pane killed; already-gone
      pane succeeds; missing identifiers → `INVALID_ARGUMENT`.
- [x] 7.2 (`plugins/session-tmux`) Implement `Tmux.ClosePane`, generalising
      `isKillPaneNotFound` into a shared target-not-found predicate also used by
      `killOriginPane`.

## 8. Documentation and verification

- [x] 8.1 (`plugins/session-tmux`) Document the four RPCs in the plugin README,
      including the focused-pane refusal and the `allow_focused` opt-out.
- [x] 8.2 (root) Run `task fmt`, `task lint`, and `task test`; all three must
      exit 0.
