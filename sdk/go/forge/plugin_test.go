...
func TestGRPCPlugin_GRPCServer(t *testing.T) {
	...
	p := &forge.GRPCPlugin{Impl: pluginv1.UnimplementedForgeServer{}}
	srv := grpc.NewServer()
	err := p.GRPCServer(nil, srv)
	require.NoError(t, err)

	info := srv.GetServiceInfo()
	require.Contains(t, info, "swm.plugin.v1.Forge")
}
...
func TestGRPCPlugin_GRPCClient(t *testing.T) {
	...
	_, ok := raw.(pluginv1.ForgeClient)
	require.True(t, ok, "GRPCClient must return a pluginv1.ForgeClient")
}
