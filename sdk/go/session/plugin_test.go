...
import (
	...
	"github.com/kalbasit/swm/sdk/go/session"
)

// Compile-time interface check: GRPCPlugin must implement goplugin.GRPCPlugin.
var _ goplugin.GRPCPlugin = (*session.GRPCPlugin)(nil)
...
func TestGRPCPlugin_GRPCServer(t *testing.T) {
	...
	p := &session.GRPCPlugin{Impl: pluginv1.UnimplementedSessionServer{}}
	srv := grpc.NewServer()
	err := p.GRPCServer(nil, srv)
	require.NoError(t, err)

	info := srv.GetServiceInfo()
	require.Contains(t, info, "swm.plugin.v1.Session")
}
...
func TestGRPCPlugin_GRPCClient(t *testing.T) {
	...
	p := &session.GRPCPlugin{}
	raw, err := p.GRPCClient(context.Background(), nil, conn)
	require.NoError(t, err)

	_, ok := raw.(pluginv1.SessionClient)
	require.True(t, ok, "GRPCClient must return a pluginv1.SessionClient")
}
